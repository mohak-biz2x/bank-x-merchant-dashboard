...
export function PremiumBuyerInvoices() {
  ...
  const doApprove = (id: string) => { setInvoices((p) => p.map((i) => i.id === id ? { ...i, status: "approved" as InvStatus, approvedDate: new Date().toISOString().split("T")[0] } : i)); showToast("success", "Invoice approved."); setConfirmApprove(null); setViewInv(null); };
    ...
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <div className="bg-white border border-gray-200 rounded">
        <table className="w-full text-sm">
          <thead><tr className="border-b border-gray-200"><th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Request ID</th><th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Supplier</th><th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Invoices</th><th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Amount</th><th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Submitted</th><th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Status</th><th className="text-left px-4 py-3 text-xs font-medium text-gray-500 uppercase">Actions</th></tr></thead>
          <tbody>{invoices.map((inv) => { const st = STATUS[inv.status]; return (<tr key={inv.id} className="border-t border-gray-100 hover:bg-gray-50"><td className="px-4 py-3 font-medium text-gray-900">{inv.id}</td><td className="px-4 py-3 text-gray-700">{inv.supplierName}</td><td className="px-4 py-3 text-gray-600 text-xs">{inv.lineItems.map((l) => l.invoiceNumber).join(", ")}</td><td className="px-4 py-3 font-medium">{fmt(inv.totalAmount)}</td><td className="px-4 py-3 text-gray-500">{inv.submittedDate}</td><td className="px-4 py-3"><span className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium ${st.color}`}><st.icon className="w-3 h-3" /> {st.label}</span></td><td className="px-4 py-3"><div className="flex gap-2"><button onClick={() => setViewInv(inv)} className="text-blue-600 text-xs font-medium flex items-center gap-1"><Eye className="w-3.5 h-3.5" /> View</button>{inv.status === "pending_approval" && (<button onClick={() => setConfirmApprove(inv)} className="text-green-600 text-xs font-medium flex items-center gap-1"><CheckCircle className="w-3.5 h-3.5" /> Approve</button>)}</div></td></tr>); })}</tbody>
        </table>
      </div>
      {viewInv && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
          <div className="bg-white rounded max-w-lg w-full max-h-[90vh] overflow-y-auto">
            <div className="px-5 py-3 flex items-center justify-between bg-[#312B6B] text-white rounded-t sticky top-0">
              <div><h3 className="text-base font-semibold text-white">Invoice Request Details</h3><p className="text-xs text-white/60 mt-0.5">{viewInv.id}</p></div>
              <button onClick={() => setViewInv(null)} className="text-white/60 hover:text-white"><X className="w-5 h-5" /></button>
            </div>
            <div className="p-5 space-y-4">
              <div className="flex items-center gap-2 mb-2"><span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium ${STATUS[viewInv.status].color}`}>{(() => { const Icon = STATUS[viewInv.status].icon; return <Icon className="w-3 h-3" />; })()}{STATUS[viewInv.status].label}</span></div>
                ...
                <div className="space-y-3">
                  {viewInv.lineItems.map((item, idx) => (
                    <div key={idx} className="border border-gray-200 rounded-lg p-3">
                      <div className="flex items-center justify-between mb-2"><p className="text-xs font-semibold text-gray-900">Invoice {idx + 1}</p><p className="text-xs font-medium text-gray-700">{fmt(item.invoiceAmount)}</p></div>
                      ...
                      <div className="grid grid-cols-2 gap-2 mt-2 text-xs">
                        <div className="flex items-center gap-1.5 p-1.5 bg-green-50 border border-green-200 rounded"><FileText className="w-3 h-3 text-green-600 flex-shrink-0" /><span className="text-green-700 truncate">{item.invoiceCopyName}</span></div>
                        <div className="flex items-center gap-1.5 p-1.5 bg-green-50 border border-green-200 rounded"><FileText className="w-3 h-3 text-green-600 flex-shrink-0" /><span className="text-green-700 truncate">{item.deliveryNoteName}</span></div>
                      </div>
                    </div>
                  ))}
                </div>
              </div>
              <div className="bg-gray-50 rounded-lg p-3 flex justify-between items-center"><span className="text-sm text-gray-600">Total ({viewInv.lineItems.length} invoice{viewInv.lineItems.length !== 1 ? "s" : ""})</span><span className="text-sm font-semibold text-gray-900">{fmt(viewInv.totalAmount)}</span></div>
              {viewInv.status === "pending_approval" ? (
                <div className="flex gap-3 pt-3 border-t border-gray-200"><button onClick={() => doReject(viewInv.id)} className="flex-1 py-2 border border-gray-300 rounded text-sm font-medium hover:bg-gray-50">Reject</button><button onClick={() => setConfirmApprove(viewInv)} className="flex-1 py-2 bg-green-600 text-white rounded text-sm font-medium hover:bg-green-700">Approve</button></div>
        ...
        <div className="fixed inset-0 bg-gray-500/30 flex items-center justify-center z-[60] p-4">
          <div className="bg-white rounded-lg shadow-xl max-w-sm w-full">
            ...
            <div className="px-6 pb-6 flex gap-3">
              ...
              <button onClick={() => doApprove(confirmApprove.id)} className="flex-1 py-2 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700">Yes, Approve</button>
            </div>
          </div>
        </div>
      )}
