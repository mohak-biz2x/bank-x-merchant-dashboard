...
export function PremiumBuyerJourney() {
  ...
  const renderStep = () => {
    switch (currentStep) {
      case 1: return (
        <div>
          <div className="flex items-center gap-3 mb-1"><div className="w-1 h-7 bg-[#4F8DFF] rounded-full"></div><h2 className="text-lg font-semibold text-gray-900">Create Your Profile</h2></div>
          ...
          <div className="bg-blue-50 border border-blue-200 rounded p-4 mb-6">
            <div className="flex gap-3"><Info className="w-5 h-5 text-blue-600 flex-shrink-0 mt-0.5" /><p className="text-sm text-blue-800">You have been invited by Mal as a Premium Buyer. Your business <span className="font-semibold">{profileData.companyLegalName}</span> has been pre-qualified for supply chain financing.</p></div>
          </div>
      ...
      case 2: return (
        <div>
          <div className="flex items-center gap-3 mb-1"><div className="w-1 h-7 bg-[#4F8DFF] rounded-full"></div><h2 className="text-lg font-semibold text-gray-900">Lite KYB Verification</h2></div>
            ...
            <>
              ...
              <div className="bg-white border border-gray-200 rounded p-5">
                ...
                <div className="flex gap-3">
                  ...
                  <button onClick={startKybVerification} disabled={!tlNumber.trim()} className="px-5 py-2.5 bg-[#4F8DFF] text-white rounded hover:bg-[#3A7AE8] disabled:opacity-50 text-sm font-medium flex items-center gap-2"><ShieldCheck className="w-4 h-4" /> Verify</button>
                </div>
              </div>
            </>
          )}
          {kybStatus === "verifying" && (
            <div className="bg-white border border-gray-200 rounded p-8 text-center">
              ...
              <div className="max-w-sm mx-auto">
                ...
                <div className="w-full h-2.5 bg-gray-200 rounded-full overflow-hidden"><div className="h-full bg-[#4F8DFF] rounded-full transition-all" style={{ width: `${kybProgress}%` }} /></div>
              </div>
            </div>
          )}
          {kybStatus === "verified" && (
      ...
      case 3: return (
        <div>
          <div className="flex items-center gap-3 mb-1"><div className="w-1 h-7 bg-[#4F8DFF] rounded-full"></div><h2 className="text-lg font-semibold text-gray-900">Terms & Conditions</h2></div>
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="bg-white border-t border-gray-200 px-8 py-4 flex items-center justify-between">
            ...
            <button onClick={handleNext} disabled={currentStep === 3 && !(acceptedTnc && acceptedPrivacy)}
              className={`px-6 py-2 rounded text-sm font-medium transition-colors ${currentStep === 3 ? (acceptedTnc && acceptedPrivacy) ? 'bg-green-600 text-white hover:bg-green-700' : 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-[#4F8DFF] text-white hover:bg-[#3A7AE8]'}`}>
              ...
              <>
                ...
                <div className="p-5">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    {!emailVerified && <button onClick={verifyEmailOtp} disabled={verifyingEmail} className="px-4 py-2 bg-[#4F8DFF] text-white rounded text-sm disabled:opacity-50 flex items-center gap-2">{verifyingEmail ? <Loader2 className="w-4 h-4 animate-spin" /> : null}{verifyingEmail ? 'Verifying...' : 'Verify'}</button>}
                  </div>
                  ...
                  <button onClick={handleOtpComplete} disabled={!emailVerified} className="w-full mt-4 py-2.5 bg-[#4F8DFF] text-white rounded text-sm font-medium disabled:opacity-50 transition-colors">Complete Verification</button>
                </div>
              </>
            ) : (
